...
extension UIView {
    ...
    func labelBold(text: String, textSize : CGFloat = 22,textColor:UIColor = .black,textNumberLine:Int = 0) -> UILabel{
        let label = UILabel()
        label.text = text
        label.textColor = textColor
        label.textAlignment = .center
        label.numberOfLines = textNumberLine
        label.font = UIFont.init(name:"HelveticaNeue-Bold",size: textSize)
        return label
    }
